...
class ZillowPropertySearch:
    ...
    async def search_property(self, location: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not properties:
                # Try to parse from raw text if structured data wasn't found
                import re
                
                # Look for patterns like addresses and prices in the result text
                ...
                # Create properties from the matched patterns
    ...
    async def get_property_details(self, property_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                desc_match = re.search(r'Description:(.*?)(?:Features:|$)', result, re.DOTALL)
                if desc_match:
                    details["description"] = desc_match.group(1).strip()
                
                # Extract features
                features_match = re.search(r'Features:(.*?)(?:Image URLs:|$)', result, re.DOTALL)
                if features_match:
                ...
                # Extract image URLs
                image_urls_match = re.findall(r'https://[^\s]+\.jpg', result)
                if image_urls_match:
